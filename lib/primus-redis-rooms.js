...
let PrimusRedisRooms = module.exports = function (primus, options) {
	...
	primus.remoteEmit = function (lookupKey, lookupValue, event, data) {
		...
		if (sparkOnThisServer) {
			...
			primus.allSpark.sendRemoteEmit(lookupKey, lookupValue, event, data);
		}
	};

	primus.remoteJoin = function (sparkLookupKey, sparkLookupValue, roomName) {
		let sparkOnThisServer = primus.findSparkByKeyValue(sparkLookupKey, sparkLookupValue);
		if (sparkOnThisServer) {
			let room = primus.room(roomName);
			sparkOnThisServer.join(room);
		}
		else {
			primus.allSpark.sendRemoteJoin(sparkLookupKey, sparkLookupValue, roomName);
		}
	}
